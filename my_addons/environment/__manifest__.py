{
    'name': 'Environment App',
    'version': '1.0',
    'depends': ['base', 'web', 'website'],
    'data': [
            'security/environment_security.xml',
            'security/ir.model.access.csv',
            'data/env.location.csv',
            'data/env.collectionunit.csv',
            'data/env.customerwastegroup.csv',
            'data/env.servicegroup.csv',
            'data/env.service.csv',
            'data/env.serviceprice.csv',
            'data/env.servicepricecomponent.csv',
            'data/env.unitservice.csv',
            'data/env.unitserviceprice.csv',
            'data/env.customer.csv',
            'views/menu.xml',
            'views/service_views.xml',
            'views/servicegroup_views.xml',
            'views/location_views.xml',
            'views/collection_unit_views.xml',
            'views/customer_views.xml',
            'views/customer_import_wizard_views.xml',
            'views/customer_waste_group_views.xml',
            'views/service_price_views.xml',
            'views/service_price_component_views.xml',
            'views/unit_service_views.xml',
            'views/unit_service_price_views.xml',
            'views/contract_views.xml',
            'views/contact_order_views.xml',
            # Website
            'views/home_templates.xml',
            'views/contracts_templates.xml',
            'views/payment_result_template.xml',
        ],
    'installable': True,
    'application': True,
}
